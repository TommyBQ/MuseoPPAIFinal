...
public class GestorDeVentaDeEntradas  {
    ...
    public GestorDeVentaDeEntradas() {
        ...
        salas[0] = new Sala("uno",20,paredes,0);
        salas[1] = new Sala("dos",25,paredes,0);
        Planta[] plantas = new Planta[2];
        plantas[0] = new Planta(1,salas,archivos,0);
        plantas[1] = new Planta(2,salas,archivos,0);
    ...
    public void crearEntrada(){
        ...
        int numeroEntrada = this.getLastEntrada() +1 ;
        ...
        // cambiar la lista de entradas a una variable local
        imprimirEntrada(entradas);
        // guardar entradas compradas + imprimir entradas
        // guardar cantidad de visitantes
        this.actualizarCantVisitantes(cantidadDeEntradas);

        finCasoUso();
    }

    private int getLastEntrada(){
        if ( entradasACrear.length > 0 ){
            Entrada last = entradasACrear[entradasACrear.length -1 ];
            return last.getNumero();
        }else {
            return 0;
        }
    }
    ...
    public void imprimirEntrada(Entrada[] entradas){
        PantallaDeVentaDeEntradas.getInstance().imprimir();

        InterfazImpresora.getInstance().imprimir(entradas);

    }

    public void actualizarCantVisitantes(int cantidadDeEntradas){
        this.cantidadPersonasEnSede += cantidadDeEntradas;
        sedeActual.imprimirCantVisitantes(this.cantidadPersonasEnSede);
        InterfazPantallaPrincipal.getInstance().imprimirCantVisitantes(this.cantidadPersonasEnSede);
